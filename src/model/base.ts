import { Pool } from "pg";
import { getDB } from "../helpers/connection";
...
class Base {
    ...
    or(conditions: { [key: string]: any }): this {
        ...
        this.orClause = conditionsKeys
            .map(
                (key, index) =>
                    `${this.tableName}."${key}" = $${this.conditionValues.length + index + 1}`
            )
            .join(" AND ");
    ...
    filter(conditions: { [key: string]: any }): this {
        ...
        this.likeClause = conditionsKeys
            .map(
                (key, index) =>
                    `${this.tableName}."${key}"::text ILIKE $${
                        this.conditionValues.length + index + 1
                    }`
            )
            .join(" AND ");
        this.conditionValues = [...this.conditionValues, ...Object.values(conditions)];
    ...
    populate(relatedTableName: string, relatedFields: string): this {
        ...
        const rField = relatedFields
            ...
            .map(
                (field) =>
                    `${relatedTableName}."${field.trim()}" AS "${relatedTableName}_${field.trim()}"`
            );
